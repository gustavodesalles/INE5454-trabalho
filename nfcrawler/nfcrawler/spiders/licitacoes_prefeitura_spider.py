...
class LicitacoesPrefeituraSpider(scrapy.Spider):
    ...
    def __init__(self, ano_inicial, ano_final, *args, **kwargs):
        super(LicitacoesPrefeituraSpider, self).__init__(*args, **kwargs)
...
def main():
    ...
    process = CrawlerProcess({
        'FEED_FORMAT': 'json',
        'FEED_URI': f'licitacoes-prefeitura-{ano_inicial if ano_inicial == ano_final else f"{ano_inicial}-{ano_final}"}.json'
    })

    process.crawl(LicitacoesPrefeituraSpider, ano_inicial=ano_inicial, ano_final=ano_final)
    process.start()
